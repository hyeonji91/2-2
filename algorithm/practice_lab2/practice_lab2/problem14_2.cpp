...
void main() {
	...
	while (testCase--) {
		...
		if (numberOfDot <= 1) {
			...
			int startIdx = getStartDotIdx();
			Dot* convexHullDot = stack;
			int convexHullDotNum = top + 1;
			initStack();
			for (int i = 0; i < convexHullDotNum; i++) {
				push(convexHullDot[(startIdx + i)% convexHullDotNum]);
			}
...
int getStartDotIdx() {
	...
	int startDotIdx = 0;
	for (int i = 0; i <= top; i++) {
		d1 = getDistance(stdDot, stack[i]);
		d2 = getDistance(stdDot, stack[(i + 1) % (top+1)]);
		d3 = getDistance(stack[i], stack[(i + 1) % (top+1)]);

		cmpDistance = d1 + d2 - d3;
		if (cmpDistance < minDistance) {
			minDistance = cmpDistance;
			startDotIdx = (i + 1) % (top + 1);
		}
	}
	return startDotIdx;
}
