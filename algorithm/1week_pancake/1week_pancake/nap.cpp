...
void main() {
	...
	} while (scheduleNum >= 100 || scheduleNum <= 0); //일정 개수가 조건에 맞을때만 다음으로 넘어가기
	...
	for (int i = 0; i < scheduleNum; i++) {
		do {
			...
			scanf("%d:%d %d:%d %s", &startAtSplit[i][0], &startAtSplit[i][1], &endAtSplit[i][0], &endAtSplit[i][1], scheduleTitle[i]);
	...
	if(maxNapTime >= 60)
		printf("the longesst nap starts at %02d:%02d and will last for %d hours and %d minutes", endAtSplit[maxNapIndex - 1][0], endAtSplit[maxNapIndex - 1][1], maxNapHour, maxNapMin);
	else
		printf("the longesst nap starts at %02d:%02d and will last for %d minutes", endAtSplit[maxNapIndex - 1][0], endAtSplit[maxNapIndex - 1][1], maxNapMin);
