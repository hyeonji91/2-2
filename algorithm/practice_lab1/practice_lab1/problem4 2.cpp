...
int main(void) {
	FILE* input;
	struct Elephant elephant[10000];
	int numOfInput = 0;
	...
	//input = fopen("elephant_input.txt", "r");
	//elephant = (struct Elephant*)malloc(sizeof(struct Elephant) * numOfInput);
	numOfInput = 0;
	while (scanf("%d %d", &elephant[numOfInput].weight, &elephant[numOfInput].iq) == 2) {
	...
	int sameCnt = 1;
	int preWeight = -1;
	qsort(elephant, numOfInput, sizeof(struct Elephant), sortAscending);
	...
	qsort(elephant, numOfInput, sizeof(struct Elephant), sortDescending);
	sameCnt = 1;
	int preIQ = -1;
	for (int i = 0; i < numOfInput; i++) {
